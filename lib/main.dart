...
class _ChatPageState extends State<ChatPage> {
  ...
  void initState() {
    super.initState();
    _loadMessages();
    if (widget.isGroup) _loadMembers();
    _connectSocket();
  }
  ...
  void _connectSocket() {
    ...
    _socket.on('online_list', (data) {
      if (!widget.isGroup && data is List) {
        setState(() {
    ...
    _socket.on('user_online', (data) {
      if (!widget.isGroup && data['userId'] == widget.userId) {
        ...
        setState(() {
          if (!_messages.any((m) => m['id'] == newId)) {
            _messages.add({
              ...
              'to_user': widget.isGroup ? null : widget.userId,
              'chat_id': widget.isGroup ? widget.chatId : null,
              'text': text,
              'image_url': '',
        ...
        title: Row(
          children: [
            AvatarWidget(
              ...
              radius: 18,
              online: _otherOnline,
              showOnline: !widget.isGroup,
              customIcon: widget.isGroup ? Icons.groups : null,
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  else if (_otherOnline && !widget.isGroup)
                    const Text(
                      'онлайн',
                      style: TextStyle(fontSize: 12, color: Colors.white70),
                    )
                  else if (widget.isGroup && _membersMap.isNotEmpty)
                    Text(
...
class _UsersPageState extends State<UsersPage> {
  ...
  Future<void> _loadStories() async {
    try {
      ...
      final data = jsonDecode(response.body);
      if (data['ok'] == true && mounted) {
        setState(() => _stories = data['stories']);
      }
    } catch (e) {}
  }
  ...
  Future<void> _loadGroups() async {
    try {
      final response = await http.get(
        Uri.parse('$serverUrl/my-chats?userId=$myUserId'),
        headers: baseHeaders,
      );
      final data = jsonDecode(response.body);
      if (data['ok'] == true && mounted) {
        setState(() => _groups = data['chats']);
      }
    } catch (e) {}
  }
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 8),
                    Text(
                      _avatarUrl.isEmpty ? 'Нажмите, чтобы выбрать фото' : 'Нажмите, чтобы изменить',
                      style: TextStyle(fontSize: 13, color: Colors.grey[600]),
                    ),
                    const SizedBox(height: 24),
                    TextField(
